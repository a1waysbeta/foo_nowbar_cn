...
static cfg_int cfg_nowbar_cover_style(
    ...
    1  // Default: Rounded (0=Square, 1=Rounded)
);
...
static cfg_int cfg_nowbar_button_accent_color(
    GUID{0xABCDEFC0, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0xC0}},
...
static cfg_int cfg_nowbar_spectrum_color2(
    GUID{0xABCDEF89, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89}},
    RGB(30, 80, 160)  // Default: darker blue (gradient bottom)
);
...
static cfg_int cfg_nowbar_waveform_color(
    GUID{0xABCDEF86, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x86}},
...
static cfg_int cfg_nowbar_waveform_style(
    GUID{0xABCDEF8C, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x8C}},
    0  // Default: Waveform 1 (0=Waveform 1 / Bottom bars, 1=Waveform 2 / Centered envelope)
);
...
static cfg_int cfg_cbutton1_glyph_size(
    GUID{0xABCDEF70, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x01}},
    80
);
static cfg_int cfg_cbutton2_glyph_size(
    GUID{0xABCDEF71, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x02}},
    80
);
static cfg_int cfg_cbutton3_glyph_size(
    GUID{0xABCDEF72, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x03}},
    80
);
static cfg_int cfg_cbutton4_glyph_size(
    GUID{0xABCDEF73, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x04}},
    80
);
static cfg_int cfg_cbutton5_glyph_size(
    GUID{0xABCDEF74, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x05}},
    80
);
static cfg_int cfg_cbutton6_glyph_size(
    GUID{0xABCDEF75, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x68, 0x06}},
    80
);
...
static cfg_int cfg_nowbar_3d_buttons(
    GUID{0xABCDEFC1, 0x1234, 0x5678, {0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0xC1}},
...
// Cached config for all 12 buttons (loaded from config file)
struct ConfigFileButtonConfig {
...
static ConfigFileButtonConfig g_config_buttons[12];
...
pfc::string8 get_config_dir_path() {
    ...
    if (profile_path.has_prefix("file://")) {
        profile_path = profile_path.subString(7);
    }
    // Ensure trailing backslash
    if (!profile_path.is_empty() && profile_path[profile_path.length() - 1] != '\\' && profile_path[profile_path.length() - 1] != '/') {
        profile_path << "\\";
    }
    pfc::string8 dir_path;
    dir_path << profile_path << "foo_nowbar_data";
    return dir_path;
}

// Get the full config file path
static pfc::string8 get_config_file_path() {
    if (g_config_file_path.is_empty()) {
        g_config_file_path = get_config_dir_path();
        g_config_file_path << "\\custom_buttons.conf";
    }
    return g_config_file_path;
}

// Helper to trim whitespace from a string
static pfc::string8 trim_string(const char* str) {
    pfc::string8 result;
...
// Parse action string to int
static int parse_action_string(const char* str) {
    pfc::string8 action = trim_string(str);
...
static void create_default_config_file() {
    ...
    file << "#\n";
    file << "# Action 可用值：none、url、executable、foobar2k、open_folder、output_device\n";
    file << "# Path:\n";
    file << "#    - 当 action 为 url 时：填写 URL\n";
    file << "#    - 当 action 为 executable 时：填写可执行文件路径\n";
    file << "#    - 当 action 为 foobar2k 时：填写菜单路径，如 '媒体库/搜索'\n";
    file << "#    - 当 action 为 open_folder 或 output_device 时：留空\n";
    file << "#\n";
    ...
    for (int i = 1; i <= 6; i++) {
        file << "[button:" << i << "]\n";
        file << "enabled = " << (get_nowbar_cbutton_enabled(i-1) ? "true" : "false") << "\n";
...
static void load_config_file() {
    ...
    std::ifstream file(wide_path.get_ptr());
    if (!file.is_open()) {
        // Config file doesn't exist - create it with current settings
        create_default_config_file();
        // Reload after creation
        file.open(wide_path.get_ptr());
        if (!file.is_open()) {
    ...
    std::string line;
    int current_button = -1;  // 0-based index
    ...
    while (std::getline(file, line)) {
        ...
        // Check for section header [button:N]
        if (line[0] == '[') {
            size_t end = line.find(']');
            if (end != std::string::npos) {
                std::string section = line.substr(1, end - 1);
                if (section.find("button:") == 0) {
                    int btn_num = std::atoi(section.substr(7).c_str());
                    if (btn_num >= 1 && btn_num <= 12) {
                        current_button = btn_num - 1;  // Convert to 0-based
                    }
                }
            }
            continue;
        ...
        if (eq_pos == std::string::npos || current_button < 0) continue;
        ...
        if (key == "enabled") {
            g_config_buttons[current_button].enabled = 
                (value == "true" || value == "1" || value == "yes");
        } else if (key == "action") {
            g_config_buttons[current_button].action = parse_action_string(value.c_str());
        } else if (key == "path") {
...
// Reload config from file (call when settings may have changed)
static void reload_config_file() {
    g_config_loaded = false;
    load_config_file();
}

// Save config file to disk (syncs buttons 1-6 from preferences, preserves 7-12 from cache)
static void save_config_file() {
    // Make sure config is loaded first so we have buttons 7-12
    if (!g_config_loaded) load_config_file();
    ...
    for (int i = 7; i <= 12; i++) {
        file << "[button:" << i << "]\n";
        file << "action = " << action_to_string(g_config_buttons[i-1].action) << "\n";
        file << "path = " << g_config_buttons[i-1].path.c_str() << "\n";
        file << "label = " << g_config_buttons[i-1].label.c_str() << "\n";
        file << "\n";
    }
    
    file.close();
    
    // Update cache for buttons 1-6 to match preferences
    for (int i = 0; i < 6; i++) {
        g_config_buttons[i].enabled = get_nowbar_cbutton_enabled(i);
...
// Get button config from config file (for buttons 7-12, always; for 1-6, optional)
static const ConfigFileButtonConfig& get_config_button(int index) {
    if (!g_config_loaded) load_config_file();
    if (index < 0 || index >= 12) {
        static ConfigFileButtonConfig empty;
        return empty;
    }
    return g_config_buttons[index];
}
...
int get_nowbar_cover_style() {
    int v = cfg_nowbar_cover_style;
    return (v >= 0 && v <= 1) ? v : 1;  // Clamp to valid range; default Rounded
}
...
COLORREF get_nowbar_spectrum_color2() {
    return static_cast<COLORREF>(cfg_nowbar_spectrum_color2.get_value());
}
...
int get_nowbar_waveform_style() {
    int s = cfg_nowbar_waveform_style;
    if (s < 0) s = 0;
    if (s > 1) s = 1;
    return s;
}
...
bool get_nowbar_smooth_animations_enabled() {
...
// Config file accessor functions for all 12 buttons
int get_config_button_action(int button_index) {
    const auto& cfg = get_config_button(button_index);
    return cfg.action;
}

pfc::string8 get_config_button_path(int button_index) {
    const auto& cfg = get_config_button(button_index);
    return cfg.path;
}

pfc::string8 get_config_button_label(int button_index) {
    const auto& cfg = get_config_button(button_index);
...
void reload_custom_button_config() {
    reload_config_file();
}
...
void nowbar_preferences::init_tab_control() {
    ...
    tie.pszText = (LPWSTR)L"图标";
    TabCtrl_InsertItem(hTab, 2, &tie);

    tie.pszText = (LPWSTR)L"自定义按钮";
    TabCtrl_InsertItem(hTab, 3, &tie);

    tie.pszText = (LPWSTR)L"字体和颜色";
...
INT_PTR CALLBACK nowbar_preferences::ConfigProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    if (msg == WM_INITDIALOG) {
        ...
        SendMessage(hCoverArtworkCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hCoverArtworkCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        // Initialize 3D Buttons combobox
        {
            ...
            SendMessage(h3dButtonsCombo, CB_ADDSTRING, 0, (LPARAM)L"启用");
            SendMessage(h3dButtonsCombo, CB_ADDSTRING, 0, (LPARAM)L"禁用");
        ...
        // Initialize Volume Number combobox
        {
            ...
            SendMessage(hVolNumCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
            SendMessage(hVolNumCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        ...
        // Initialize Playback Time combobox
        {
            ...
            SendMessage(hPlaybackTimeCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
            SendMessage(hPlaybackTimeCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hSeekbarVisibleCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hSeekbarVisibleCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hSeekbarLengthCombo, CB_ADDSTRING, 0, (LPARAM)L"固定");
        SendMessage(hSeekbarLengthCombo, CB_ADDSTRING, 0, (LPARAM)L"缩放");
        ...
        SendMessage(hCoverStyleCombo, CB_SETCURSEL, get_nowbar_cover_style(), 0);
        ...
        SendMessage(hShuffleIconCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hShuffleIconCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hRepeatIconCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hRepeatIconCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hRatingStarsCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hRatingStarsCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        SendMessage(hRatingStarsCombo, CB_ADDSTRING, 0, (LPARAM)L"第三行");
        {
        ...
        SendMessage(hStopAfterCurrentCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hStopAfterCurrentCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hSuperIconCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hSuperIconCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hAlternateIconsCombo, CB_ADDSTRING, 0, (LPARAM)L"样式 1");
        SendMessage(hAlternateIconsCombo, CB_ADDSTRING, 0, (LPARAM)L"样式 2");
        SendMessage(hAlternateIconsCombo, CB_ADDSTRING, 0, (LPARAM)L"样式 3");
        ...
        SendMessage(hPlayIconStyleCombo, CB_ADDSTRING, 0, (LPARAM)L"正常");
        SendMessage(hPlayIconStyleCombo, CB_ADDSTRING, 0, (LPARAM)L"反相");
        ...
        SendMessage(hVolumeIconCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hVolumeIconCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hVolumeBarCombo, CB_ADDSTRING, 0, (LPARAM)L"显示");
        SendMessage(hVolumeBarCombo, CB_ADDSTRING, 0, (LPARAM)L"隐藏");
        ...
        SendMessage(hSkipLowRatingCombo, CB_ADDSTRING, 0, (LPARAM)L"禁用");
        SendMessage(hSkipLowRatingCombo, CB_ADDSTRING, 0, (LPARAM)L"启用");
        ...
        SendMessage(hSkipRatingThresholdCombo, CB_ADDSTRING, 0, (LPARAM)L"1（等级 = 1 时跳过）");
        SendMessage(hSkipRatingThresholdCombo, CB_ADDSTRING, 0, (LPARAM)L"2（等级 ≤ 2 时跳过）");
        SendMessage(hSkipRatingThresholdCombo, CB_ADDSTRING, 0, (LPARAM)L"3（等级 ≤ 3 时跳过）");
        ...
        // Initialize Visualization section
        {
            ...
            SendMessage(hSpecWidth, CB_ADDSTRING, 0, (LPARAM)L"细");
            SendMessage(hSpecWidth, CB_ADDSTRING, 0, (LPARAM)L"正常");
            SendMessage(hSpecWidth, CB_ADDSTRING, 0, (LPARAM)L"宽");
            ...
            SendMessage(hSpecStyle, CB_ADDSTRING, 0, (LPARAM)L"条形");
            SendMessage(hSpecStyle, CB_ADDSTRING, 0, (LPARAM)L"曲线");
            SendMessage(hSpecStyle, CB_ADDSTRING, 0, (LPARAM)L"多米诺");
            ...
            SendMessage(hSpecHeight, CB_ADDSTRING, 0, (LPARAM)L"低");
            SendMessage(hSpecHeight, CB_ADDSTRING, 0, (LPARAM)L"正常");
            SendMessage(hSpecHeight, CB_ADDSTRING, 0, (LPARAM)L"高");
            ...
            SendMessage(hOpacitySlider, TBM_SETPOS, TRUE, cfg_nowbar_spectrum_opacity);
            {
                wchar_t buf[8];
                wsprintfW(buf, L"%d%%", (int)cfg_nowbar_spectrum_opacity);
            ...
            // Initialize color mode combo
            {
                ...
                SendMessage(hColorMode, CB_ADDSTRING, 0, (LPARAM)L"自适应");
                SendMessage(hColorMode, CB_ADDSTRING, 0, (LPARAM)L"纯色");
                SendMessage(hColorMode, CB_ADDSTRING, 0, (LPARAM)L"渐变");
                SendMessage(hColorMode, CB_ADDSTRING, 0, (LPARAM)L"频率");
            ...
            SendMessage(hWaveWidth, CB_ADDSTRING, 0, (LPARAM)L"细");
            SendMessage(hWaveWidth, CB_ADDSTRING, 0, (LPARAM)L"宽");
            SendMessage(hWaveWidth, CB_ADDSTRING, 0, (LPARAM)L"宽（4K）");
            SendMessage(hWaveWidth, CB_SETCURSEL, cfg_nowbar_waveform_width, 0);

            // Initialize waveform style radio buttons (1 vs 2)
            CheckRadioButton(hwnd, IDC_VIS_WAVEFORM_STYLE_1, IDC_VIS_WAVEFORM_STYLE_2,
                (cfg_nowbar_waveform_style == 1) ? IDC_VIS_WAVEFORM_STYLE_2 : IDC_VIS_WAVEFORM_STYLE_1);
        ...
        for (int id : cbutton_action_combos) {
            ...
            SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)L"打开所在文件夹");
            SendMessage(hCombo, CB_ADDSTRING, 0, (LPARAM)L"输出设备");
        }
        ...
        // Initialize About tab text
        {
            ...
            // 正确转换 UTF-8 -> UTF-16
            MultiByteToWideChar(CP_UTF8, 0, FOO_NOWBAR_DESCRIPTION, -1, about_desc, 256);
    ...
    case WM_COMMAND:
        ...
        case IDC_VIS_WAVEFORM_STYLE_1:
        case IDC_VIS_WAVEFORM_STYLE_2:
            if (HIWORD(wp) == BN_CLICKED) {
                CheckRadioButton(hwnd, IDC_VIS_WAVEFORM_STYLE_1, IDC_VIS_WAVEFORM_STYLE_2, LOWORD(wp));
                if (IsDlgButtonChecked(hwnd, IDC_VIS_WAVEFORM_RADIO) != BST_CHECKED) {
                    CheckRadioButton(hwnd, IDC_VIS_SPECTRUM_RADIO, IDC_VIS_WAVEFORM_RADIO, IDC_VIS_WAVEFORM_RADIO);
                }
                update_vis_section_state(hwnd);
                p_this->on_changed();
            }
        ...
        case IDC_SPECTRUM_COLOR2_BTN:
            if (HIWORD(wp) == BN_CLICKED) {
                COLORREF color = static_cast<COLORREF>(cfg_nowbar_spectrum_color2.get_value());
                if (show_color_picker(hwnd, color)) {
                    cfg_nowbar_spectrum_color2 = color;
                    InvalidateRect(GetDlgItem(hwnd, IDC_SPECTRUM_COLOR2_BTN), nullptr, TRUE);
                    p_this->on_changed();
                }
            }
            break;
    ...
    case WM_HSCROLL:
        if ((HWND)lp == GetDlgItem(hwnd, IDC_SPECTRUM_OPACITY_SLIDER)) {
            int pos = (int)SendMessage(GetDlgItem(hwnd, IDC_SPECTRUM_OPACITY_SLIDER), TBM_GETPOS, 0, 0);
            wchar_t buf[8];
    ...
    case WM_DRAWITEM:
        {
            ...
            if (dis->CtlID == IDC_BUTTON_ACCENT_BTN || dis->CtlID == IDC_PROGRESS_ACCENT_BTN ||
                ...
                dis->CtlID == IDC_SPECTRUM_COLOR2_BTN) {
                COLORREF color;
                ...
                } else if (dis->CtlID == IDC_SPECTRUM_COLOR2_BTN) {
                    color = static_cast<COLORREF>(cfg_nowbar_spectrum_color2.get_value());
                } else {
...
void nowbar_preferences::apply_settings() {
    if (m_hwnd) {
        ...
        // Save visualization mode from checkbox + radio buttons
        {
            ...
            cfg_nowbar_waveform_style = (IsDlgButtonChecked(m_hwnd, IDC_VIS_WAVEFORM_STYLE_2) == BST_CHECKED) ? 1 : 0;
        ...
        // Save current settings to the current profile
        {
            ...
            // Sync buttons 1-6 to the external config file
            save_config_file();
        }
...
void nowbar_preferences::reset_settings() {
    if (m_hwnd) {
        if (m_current_tab == 0) {
            ...
            CheckRadioButton(m_hwnd, IDC_VIS_WAVEFORM_STYLE_1, IDC_VIS_WAVEFORM_STYLE_2, IDC_VIS_WAVEFORM_STYLE_1);
            ...
            cfg_nowbar_cover_style = 1;  // Rounded
            ...
            cfg_nowbar_spectrum_opacity = 75;
            cfg_nowbar_spectrum_gradient_mode = 0;
            cfg_nowbar_spectrum_color2 = RGB(30, 80, 160);
            ...
            InvalidateRect(GetDlgItem(m_hwnd, IDC_SPECTRUM_COLOR2_BTN), nullptr, TRUE);
